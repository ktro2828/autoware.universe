...
#ifndef AUTOWARE__MTR__ARCHETYPE__RESULT_HPP_
#define AUTOWARE__MTR__ARCHETYPE__RESULT_HPP_
...
public:
   ...
   */
  explicit Result(const MTRError & error) : value_(error) {}
  ...
  /**
   * @brief Return an expected value if it holds, overwise throw `mtreException`.
   */
  T unwrap() const
  {
    if (is_ok()) {
      ...
      throw MTRException(std::get<MTRError>(value_));
    }
  }

private:
  std::variant<T, MTRError> value_;  //!< Container of expected value or error.
};
 ...
 */
template <typename T>
Result<T> Err(const MTRError & error) noexcept
{
 ...
 */
template <typename T>
Result<T> Err(const MTRError_t & kind) noexcept
{
  MTRError error(kind);
  return Result<T>(error);
}
 ...
 */
template <typename T>
Result<T> Err(const MTRError_t & kind, const std::string & msg) noexcept
{
  MTRError error(kind, msg);
  return Result<T>(error);
}
}  // namespace autoware::mtr::archetype
#endif  // AUTOWARE__MTR__ARCHETYPE__RESULT_HPP_
