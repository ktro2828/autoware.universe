...
#ifndef AUTOWARE__TENSORRT_PLUGINS__POINT_ROPE_PLUGIN_HPP_
#define AUTOWARE__TENSORRT_PLUGINS__POINT_ROPE_PLUGIN_HPP_
...
{
constexpr char const * const kPOINT_ROPE_PLUGIN_NAME{"PointRoPE"};
constexpr char const * const kPOINT_ROPE_PLUGIN_VERSION{"1"};
constexpr char const * const kPOINT_ROPE_PLUGIN_NAMESPACE{""};

struct PointRoPEParameters
{
  float base;
  float f0;
};

class PointRoPEPlugin : public IPluginV3,
                        public IPluginV3OneCore,
                        public IPluginV3OneBuild,
                        public IPluginV3OneRuntime
{
public:
  explicit PointRoPEPlugin(const std::string & name, const PointRoPEParameters & params) noexcept;

  ~PointRoPEPlugin() override = default;
...
private:
  ...
  PointRoPEParameters params_;
  std::vector<PluginField> data_to_serialize_;
  PluginFieldCollection fc_to_serialize_;
};
}  // namespace nvinfer1::plugin
#endif  // AUTOWARE__TENSORRT_PLUGINS__POINT_ROPE_PLUGIN_HPP_
