...
#ifndef TENSORRT__COMMON__CHECKMACROSPLUGIN_H_  // NOLINT
#define TENSORRT__COMMON__CHECKMACROSPLUGIN_H_  // NOLINT
...
template <ILogger::Severity kSeverity>
inline LogStream<kSeverity> & operator<<(
  LogStream<kSeverity> & stream, std::ostream & (*f)(std::ostream &))
{
  std::lock_guard<std::mutex> guard(stream.getMutex());
  auto & os = static_cast<std::ostream &>(stream);
  os << f;
...
#define PLUGIN_API_CHECK_ENUM_RANGE(Type, val) \
  PLUGIN_API_CHECK(static_cast<int>(val) >= 0 && static_cast<int>(val) < EnumMax<Type>())
#define PLUGIN_API_CHECK_ENUM_RANGE_RETVAL(Type, val, retval) \
  PLUGIN_API_CHECK_RETVAL(                                    \
    static_cast<int>(val) >= 0 && static_cast<int>(val) < EnumMax<Type>(), retval)
...
#define PLUGIN_CUERROR(status_)                                                                 \
  {                                                                                             \
    auto s_ = status_;                                                                          \
    if (s_ != 0) nvinfer1::plugin::logError(#status_ " failure.", __FILE__, FN_NAME, __LINE__); \
  }

#endif  // TRT_TUT_HELPERS_H
#endif  // TRT_CHECK_MACROS_H

#endif  // TENSORRT__COMMON__CHECKMACROSPLUGIN_H_
